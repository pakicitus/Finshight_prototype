...
def resolve_goal_id_from_db(
    ...
    goal_by_id = (
        db.query(Goal)
        .filter(Goal.user_id == str(user_id), Goal.id == str(goal_name_or_id))
        .first()
    )
    if goal_by_id:
    ...
    goals = db.query(Goal).filter(Goal.user_id == str(user_id)).all()
    query_name = goal_name_or_id.lower().strip()
...
def run_finSight_pipeline(
    ...
    try:
        ...
        if active_db and "goals" not in context_dict:
            try:
                db_goals = active_db.query(Goal).filter(Goal.user_id == str(user_id)).all()
                if db_goals:
        ...
        try:
            ...
            if func_name == "get_balance":
                if accepts_db:
                    engine_result = engine_fn(user_id=user_id, db=active_db)
                else:
                    engine_result = engine_fn(user_id=user_id)

            elif func_name == "get_spending_summary":
                period = args.get("period", "this_month")
                if accepts_db:
                    engine_result = engine_fn(user_id=user_id, db=active_db, period=period)
                else:
                    cat = args.get("category")
                    if "category" in fn_sig.parameters and cat:
                        engine_result = engine_fn(user_id=user_id, period=period, category=cat)
                    else:
                        engine_result = engine_fn(user_id=user_id, period=period)

            elif func_name == "check_affordability":
                amount = args.get("amount")
                if accepts_db:
                    engine_result = engine_fn(user_id=user_id, amount=amount, db=active_db)
                else:
                    item_desc = args.get("item_description")
                    if "item_description" in fn_sig.parameters and item_desc:
                        engine_result = engine_fn(user_id=user_id, amount=amount, item_description=item_desc)
                    else:
                        engine_result = engine_fn(user_id=user_id, amount=amount)
                ...
                if not goal_id and active_db:
                    goal_name_or_id = args.get("goal_name") or args.get("goal_name_or_id", "")
                    goal_id = resolve_goal_id_from_db(user_id, goal_name_or_id, active_db)
